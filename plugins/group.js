...
async function handleRateLimit() {
  ...
  if (timeSinceLastRequest < rateLimit.minDelay) {
    const delay = rateLimit.minDelay - timeSinceLastRequest;
...
bot(
  ...
  async (message, bot) => {
    const fs = require('fs');
    const path = require('path');
    const configPath = path.join(__dirname, '..', 'config.js'); // Changed to parent directory
    ...
    try {
      ...
      if (message.query === 'on') {
        // Update GREETING to "true" in config.js
        ...
        // Update GREETING to "false" in config.js
        configContent = configContent.replace(
          /GREETING: "(true|false)"/, 
          'GREETING: "false"'
        );
        ...
        // Extract current greeting status
      ...
      await bot.reply('Failed to update greeting configuration. Please check server logs.');
    }
  }
);
...
bot(
  ...
  async (message, bot) => {
    ...
    try {
      await handleRateLimit(); // Add rate limiting
...
bot(
  ...
  async (message, bot) => {
    ...
    try {
      // Check if bot is admin (only admins can get invite links)
      const metadata = await bot.sock.groupMetadata(message.chat);
      const isAdmin = metadata.participants.find(p => p.id === message.sender)?.admin;
      ...
      // Send the link with a mention to the requester
      await bot.reply(
        `*Group Invite Link:*\n\n${inviteLink}\n\n`);
...
bot(
  ...
  async (message, bot) => {
    ...
    try {
      // Check if bot is admin (only admins can revoke links)
      const metadata = await bot.sock.groupMetadata(message.chat);
      const isAdmin = metadata.participants.find(p => p.id === message.sender)?.admin;
    ...
    try {
      ...
      if (!code || code.length < 10) {
        return await bot.reply(
          `*Invalid invite link!*\n\n` +
          `Please send a valid WhatsApp group invite link or code.\n` +
          `Example: *!join https://chat.whatsapp.com/EXAMPLE123*`
        );
      }
      ...
      // Get group metadata to show name
      ...
      await bot.reply(
        ...
        `- Group is full\n\n` +
        `_Error details: ${error.message || 'Unknown error'}_`
      );
    }
  }
);
...
bot(
  ...
  async (message, bot) => {
    try {
      ...
      // Send message with group picture if available
      if (groupPicUrl) {
...
async function handleRequestAction(message, bot, action) {
  try {
    ...
    await bot.reply(
      `*Failed to ${action} join request(s)!*\n\n` +
      `_Error details: ${error.message || 'Unknown error'}_`
    );
  }
}
...
bot(
  {
    name: 'gmode',
    info: 'Change group member add mode (admin or member)',
    category: "group",
    usage: 'gmode <admin/member>'
  },
  async (message, bot) => {
    if (!message.chat.endsWith('@g.us')) {
      return await bot.reply('_This command only works in groups!_');
...
bot(
  {
    ...
    category: "group",
    usage: 'newgc <groupname> | <number1>,<number2>,...'
  },
  async (message, bot) => {
    const query = message.query;
    if (!query) {
      return await bot.reply('*Usage:* newgc <groupname> | <number1>,<number2>,...\n_Example: newgc MyGroup | 1234567890,9876543210_');
    }
...
// gtime command - change ephemeral message duration
bot(
  {
    name: 'gtime',
    info: 'Change message disappearing timer in group',
    category: "group",
    usage: 'gtime <0/24/7/90>'
  },
  async (message, bot) => {
    if (!message.chat.endsWith('@g.us')) {
      return await bot.reply('_This command only works in groups!_');
...
// rgpp - Remove group profile picture
bot(
  {
    name: 'rgpp',
  ...
  async (message, bot) => {
    ...
    try {
      ...
      await bot.reply(
        `*Failed to remove group profile picture!*\n` +
        `_Error: ${error.message || 'Unknown error'}_`
      );
    }
  }
...
bot(
  ...
  async (message, bot) => {
    ...
    try {
      ...
      await bot.reply(
        `*Failed to get group profile picture!*\n` +
        `_Error: ${error.message || 'Unknown error'}_`
      );
    }
  }
  ...
  async (message, bot) => {
    ...
    try {
      ...
      await bot.sock.sendMessage(
        ...
        {
          text: text,
          mentions: participants,  // Mention all participants
          contextInfo: {
            mentionedJid: participants  // Additional mention metadata
          }
        }
      );
    } catch (error) {
...
// Initialize antiword.json if it doesn't exist
...
function setupWatchers() {
    // Stop existing watchers if they exist
    if (configWatcher) configWatcher.close();
    if (antiwordWatcher) antiwordWatcher.close();
    
    // Watch for config changes
    configWatcher = fs.watch(path.join(__dirname, '../config.js'), (eventType, filename) => {
        if (eventType === 'change') {
            try {
    ...
    // Watch for antiword.json changes
    antiwordWatcher = fs.watch(antiwordPath, (eventType, filename) => {
        if (eventType === 'change') {
            try {
                const newState = JSON.parse(fs.readFileSync(antiwordPath, 'utf-8'));
                antiwordState = newState;
            } catch (error) {
                console.error('Error reloading antiword.json:', error);
            }
        }
    });
}
...
function isPrivileged(senderJid) {
    ...
    try {
        const sudoData = JSON.parse(fs.readFileSync(path.join(jsonDir, 'sudo.json'), 'utf-8'));
        const sudoNumbers = sudoData?.sudo?.map(cleanJid) || [];
        return cleanedSender === ownerNumber || sudoNumbers.includes(cleanedSender);
    } catch (error) {
...
async function checkGroup(message) {
    if (!message.chat.endsWith('@g.us')) {
        await message.send('_This command only works in groups!_');
        return false;
    }
    return true;
}
...
bot(
    {
        ...
        usage: [
            ...
            "antiword remove [word] - Remove a word from filter",
            "antiword list - Show filtered words",
            "antiword on - Enable antiword",
            "antiword off - Disable antiword",
            "antiword mode [warn|delete|kick] - Set action"
        ]
    },
    async (message, bot) => {
        if (!await checkGroup(message)) return;

        const metadata = await bot.sock.groupMetadata(message.chat);
        const isAdmin = metadata.participants.find(p => p.id === message.sender)?.admin;
        
        if (!isPrivileged(message.sender) && !isAdmin) {
        ...
        switch (action) {
            ...
            case 'mode':
                ...
                if (isPrivileged(message.sender)) {
                    // Update global config if owner/sudo
...
bot(
    {
        name: "antilink",
        info: "Manages antilink settings",
        category: "group",
        usage: [
            "antilink [on|off] - Toggle antilink",
            "antilink mode [warn|delete|kick] - Set action"
        ]
    },
    async (message, bot) => {
        if (!await checkGroup(message)) return;
        ...
        const enabled = config.ANTILINK === "true";
        const mode = config.ANTILINK_ACTION || "warn";

        switch (action) {
            case 'on':
                const onSuccess = updateConfig({ ANTILINK: "true" });
                if (onSuccess) {
                    return await message.send("Antilink has been enabled globally.");
                } else {
                    return await message.send("Failed to enable antilink.");
                }
                
            case 'off':
                const offSuccess = updateConfig({ ANTILINK: "false" });
                if (offSuccess) {
                    return await message.send("Antilink has been disabled globally.");
                } else {
                    return await message.send("Failed to disable antilink.");
                }
                
            case 'mode':
                const newMode = message.args[1]?.toLowerCase();
                if (!newMode || !['warn', 'delete', 'kick'].includes(newMode)) {
                    return await message.send("Usage: antilink mode [warn|delete|kick]");
                }
                
                const modeSuccess = updateConfig({ ANTILINK_ACTION: newMode });
                if (modeSuccess) {
                    return await message.send(`Set antilink action to "${newMode}".`);
                } else {
                    return await message.send("Failed to update antilink action.");
                }
                
            default:
                return await message.send(
                    `Antilink Status: ${enabled ? 'ON' : 'OFF'}\n` +
                    `Current Action: ${mode}\n\n` +
                    `Usage:\n` +
                    `${config.PREFIX}antilink on - Enable antilink\n` +
                    `${config.PREFIX}antilink off - Disable antilink\n` +
                    `${config.PREFIX}antilink mode [warn|delete|kick] - Set action`
                );
        }
    }
);
...
bot(
    {
        name: "antibot",
        info: "Manages antibot settings",
        category: "group",
        usage: [
            "antibot [on|off] - Toggle antibot",
            "antibot mode [warn|delete|kick] - Set action"
        ]
    },
    async (message, bot) => {
        if (!await checkGroup(message)) return;
        ...
        const enabled = config.ANTIBOT === "true";
        const mode = config.ANTIBOT_ACTION || "warn";

        switch (action) {
            case 'on':
                const onSuccess = updateConfig({ ANTIBOT: "true" });
                if (onSuccess) {
                    return await message.send("Antibot has been enabled globally.");
                } else {
                    return await message.send("Failed to enable antibot.");
                }
                
            case 'off':
                const offSuccess = updateConfig({ ANTIBOT: "false" });
                if (offSuccess) {
                    return await message.send("Antibot has been disabled globally.");
                } else {
                    return await message.send("Failed to disable antibot.");
                }
                
            case 'mode':
                const newMode = message.args[1]?.toLowerCase();
                if (!newMode || !['warn', 'delete', 'kick'].includes(newMode)) {
                    return await message.send("Usage: antibot mode [warn|delete|kick]");
                }
                
                const modeSuccess = updateConfig({ ANTIBOT_ACTION: newMode });
                if (modeSuccess) {
                    return await message.send(`Set antibot action to "${newMode}".`);
                } else {
                    return await message.send("Failed to update antibot action.");
                }
                
            default:
                return await message.send(
                    `Antibot Status: ${enabled ? 'ON' : 'OFF'}\n` +
                    `Current Action: ${mode}\n\n` +
                    `Usage:\n` +
                    `${config.PREFIX}antibot on - Enable antibot\n` +
                    `${config.PREFIX}antibot off - Disable antibot\n` +
                    `${config.PREFIX}antibot mode [warn|delete|kick] - Set action`
                );
        }
    }
);
...
async function handleModeration(mode, type, message, bot) {
    ...
    // Delete the message first if config.DELETE is true
    ...
    switch (mode) {
        case 'warn':
            // Initialize warn counter if not exists
        ...
        case 'delete':
            // Initialize delete counter if not exists
...
// Message listener for antiword, antilink, antibot
bot(
    {
        on: 'text',
    ...
    async (message, bot) => {
        try {
            ...
            // Skip if message is from privileged user or admin
            if (isPrivileged(sender)) return;
            ...
            if (error.message.includes('rate-overLimit')) {
                ...
                // Increase delay time temporarily
                rateLimit.minDelay = 3000; // 3 seconds
                setTimeout(() => {
                    rateLimit.minDelay = 1000; // Reset after 1 minute
                }, 60000);
            } else {
