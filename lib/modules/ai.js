...
async function aigf(prompt, userId = null) {
  try {
    ...
    if (userId) {
      try {
        const { data } = await axios.post(`${KING_URL}/ai/aigf`, {
          userId: userId,
          prompt: prompt,
        });
    ...
    const { data } = await axios.get(`${KING_URL}/ai/aigf?prompt=${encodeURIComponent(prompt)}`);
    if (data && data.status && data.response) {
      return data.response;
    } else {
...
async function gpt(prompt) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/gpt?prompt=${encodeURIComponent(prompt)}`);
    if (data && data.status && data.response) {
      return data.response;
    } else {
...
async function groq(prompt) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/groq?prompt=${encodeURIComponent(prompt)}`);
    if (data && data.status && data.response) {
      return data.response;
    } else {
...
async function powerbrain(question) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/powerbrain?question=${encodeURIComponent(question)}`);
    if (data && data.status && data.response) {
...
async function xai(prompt) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/xai?prompt=${encodeURIComponent(prompt)}`);
    if (data && data.status && data.response) {
      return data.response;
    } else {
...
async function gemini(prompt) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/gemini?prompt=${encodeURIComponent(prompt)}`);
    if (data && data.status && data.response) {
...
async function blackbox(query) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/blackbox?query=${encodeURIComponent(query)}`);
    if (data && data.status && data.response) {
      return data.response;
    } else {
...
async function tts(text) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/tts?text=${encodeURIComponent(text)}&lang=en`);
...
async function realistic(prompt) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/aicreate/realistic?prompt=${encodeURIComponent(prompt)}`);
...
async function flux(prompt) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/aicreate/flux?prompt=${encodeURIComponent(prompt)}`);
    if (data && data.status && data.response && Array.isArray(data.response) && data.response.length > 0) {
      return data.response[0];
    } else {
      return null;
...
async function deepimg(prompt, style = "Fantasy 3D") {
  try {
    const { data } = await axios.get(`${KING_URL2}/ai/deepimg?prompt=${encodeURIComponent(prompt)}&style=${encodeURIComponent(style)}`);
...
async function geminiUpload(prompt, image, mimeType) {
  try {
    ...
    const { data } = await axios.post(`${KING_URL}/ai/gemini-upload`, form, {
      headers: {
        ...form.getHeaders(),
...
async function alya(prompt) {
  try {
    const { data } = await axios.get(`${KING_URL}/ai/alya?prompt=${encodeURIComponent(prompt)}`);
    if (data && data.status && data.response) {
      return data.response;
    } else {
      return "I'm having some trouble accessing my extended intelligence. Could you try asking differently?";
...
async function resize(image, options = {}) {
  try {
    ...
    const { data } = await axios.post(`${KING_URL}/tools/resize`, form, {
      headers: {
        ...form.getHeaders(),
        'Content-Length': form.getLengthSync()
      },
...
async function rotate(image, direction = 'right') {
    try {
        ...
        const { data } = await axios.post(`${KING_URL}/tools/rotate`, form, {
            params: { direction },
            headers: {
                ...form.getHeaders(),
                'Content-Length': form.getLengthSync()
        ...
        return {
            status: false,
            message: `Error rotating image: ${err.response?.data?.message || err.message}`
        };
    }
}
module.exports = {
