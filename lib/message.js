...
class WhatsAppBot {
    ...
    const chat = msg.chat || msg.key?.remoteJid || null;
    const sender = msg.sender || msg.key?.participant || chat;

   // console.log('chat:', chat);
    // console.log('key:', msg.key);

    // Now proxy everything
    ...
    this.chat = chat;
    this.sender = sender;
    this.isGroup = msg.isGroup || getChatType(chat) === 'group';
    ...
    async reply(text, opt = { withTag: true }) {
        ...
        return this.sendMessage(this.chat, {
            ...
            ...opt, 
            quoted: opt.withTag ? nikk : this._currentMessage 
        });
    }
...
async sendImageAsSticker(jid, path, quoted, options = {}) {
  ...
  .then(response => {
    ...
    // Convenience method to create and send sticker from image
    ...
    // Convenience method to create and send sticker from video
    ...
    // Convenience method to create and send sticker from existing webp
