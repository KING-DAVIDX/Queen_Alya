...
class MenuModule {
    ...
    async sendV3Menu(message, menuContent) {
        const videoUrl = this.getRandomMedia('video');
        await this.sock.sendMessage(message.chat, {
            video: { url: videoUrl },
            caption: menuContent.text
        });
    ...
    async sendV9Menu(message, menuContent) {
        const videoUrl = this.getRandomMedia('video');
        const imageUrl = this.getRandomMedia('image');
        await this.sock.sendMessage(message.chat, {
            video: { url: videoUrl },
    ...
    async sendV10Menu(message, menuContent) {
        const buttons = menuContent.buttons.map(btn => ({
            name: "quick_reply",
            buttonParamsJson: JSON.stringify({
    ...
    async handleMenuCommand(message) {
        try {
            ...
            switch (this.menuTypeCache) {
                ...
                case 'v9': return this.sendV9Menu(message, menuContent);
                case 'v10': return this.sendV10Menu(message, menuContent);
    ...
    async handleMenuTypeCommand(message) {
        try {
            ...
            if (newType !== "v1" && newType !== "v2" && newType !== "v3" && 
                ...
                newType !== "v7" && newType !== "v8" && newType !== "v9" &&
                newType !== "v10") {
                return await this.sock.sendMessage(message.chat, { 
                    text: "❌ *Invalid menu type!*\nPlease use `menutype v1` to `menutype v10`" 
                });
            }
