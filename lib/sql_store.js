...
class sql_store {
        ...
        async initialize() {
                try {
                        ...
                        console.log('SQLite store initialized successfully');
                } catch (error) {
                        console.error('Error initializing SQLite store:', error);
                        throw error;
                }
        }
        ...
        while (attempt < maxAttempts) {
            try {
                ...
                do {
                    ...
                    // Log progress
                    if (deleted > 0) {
                        console.log(`Batch deleted ${deleted} messages. Total: ${totalDeleted}`);
                    }

                    // Check if operation timeout exceeded
                    if (Date.now() - startTime > timeout) {
                        console.log('Operation timeout reached. Will resume in next run.');
                        break;
                    }
                ...
                if (totalDeleted >= vacuumThreshold) {
                    try {
                        ...
                        console.warn('Database optimization failed:', optimizeError);
                        // Continue execution even if optimization fails
                    }
                }

                console.log(`Successfully cleared ${totalDeleted} old messages`);
                return {
                    success: true,
