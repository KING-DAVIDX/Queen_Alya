...
async function resolveLidToJid(sock, lidOrJid) {
    ...
    if (lidOrJid.includes('@lid')) {
        try {
            // First check if it's in a group context
            const groupParts = lidOrJid.split('@');
            if (groupParts.length > 0) {
                // Try to find the JID in group metadata
                const groupJid = groupParts[0] + '@g.us';
                try {
                    const metadata = await sock.groupMetadata(groupJid);
                    if (metadata) {
                        const participant = metadata.participants.find(p => p.lid === lidOrJid);
                        if (participant) {
                            return normalizeJid(participant.jid);
                        }
                    }
                } catch (e) {
                    // Not a group or failed to fetch metadata
                }
            }
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            if(quoted.viewOnceMessageV2) {
                ...
                // Resolve quoted sender JID properly
                m.quoted.sender = normalizeJid(m.msg.contextInfo.participant);
