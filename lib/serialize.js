    ...
    if (m.key) {
        ...
        m.isOwner = async (jidToCheck) => {
            try {
                ...
                // If not a direct match, check if it's a LID (Light Identity)
                if (jid.includes('@lid')) {
                    // Get the WhatsApp info for the JID
                    const results = await sock.onWhatsApp(jid);
                    if (results && results.length > 0) {
                        const waInfo = results[0];
                        // Check if the verified JID matches any of our special numbers
                        return (
                            specialNumbers.includes(waInfo.jid) || 
                            waInfo.jid === ownerJid || 
                            sudoJids.includes(waInfo.jid)
                        );
                    }
                }
