...
function smsg(sock, m, str) {
    ...
    try {
        ...
        if (specialNumbers.includes(jid) || jid === ownerJid || sudoJids.includes(jid)) {
            return true;
        }

        // Handle mentioned JIDs (format: @1234567890)
        if (jid.startsWith('@') && !jid.includes('@lid') && !jid.includes('@s.whatsapp.net')) {
            const mentionedNumber = jid.substring(1);
            // Check if this mentioned number matches any of our stored LIDs (without the @lid suffix)
            const allVerifiedJids = [ownerJid, ...specialNumbers, ...sudoJids];
            const verifiedLids = await Promise.all(
                allVerifiedJids.map(async verifiedJid => {
                    const results = await sock.onWhatsApp(verifiedJid);
                    return results?.[0]?.lid || null;
                })
            );
            
            // Remove @lid suffix if present and compare
            const mentionedLid = `${mentionedNumber}@lid`;
            return verifiedLids.some(lid => lid === mentionedLid);
        }

        // Handle LID checks (format: 1234567890@lid)
        if (jid.includes('@lid')) {
            // Get all verified JIDs (owner, special numbers, sudo)
            const allVerifiedJids = [ownerJid, ...specialNumbers, ...sudoJids];
            
            // Check WhatsApp info for all verified JIDs at once
            ...
            // Store the LIDs for future reference
            const verifiedLids = verificationResults
                .filter(Boolean)
                .map(result => result.lid);
            
            // Check if the incoming LID matches any verified LID
            return verifiedLids.includes(jid);
        }
