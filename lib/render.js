...
async function setEnvVar(key, value) {
  ...
  try {
    ...
    watchConfigForEnvVar(key, configPath);

    await restartService();
    return {
      success: true,
      data: response.data,
      message: `Environment variable ${key} set successfully and ${configPath} is now being watched for changes`
    };
  } catch (err) {
...
function watchConfigForEnvVar(key, configPath) {
    ...
    if (event === 'change') {
      try {
        ...
        if (config[key] !== undefined) {
          console.log(`Detected change in config.js for ${key}, updating environment variable...`);
